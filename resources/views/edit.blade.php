        ...
        <div><a class="btn btn-primary" href="{{ route('home')}}">Back</a></div>
            ...
            @endif
                ...
                <div class="form-group">
                    ...
                    <input type="text" name="name" value="{{$task->name}}" class="form-control" id="exampleInputEmail1" aria-describedby="emailHelp" placeholder="Task name">
                </div>
                <div class="form-group">
                    <label for="exampleInputPassword1">Priority</label>
                    <select name="priority" class="form-control">
                        <option value="">Select Priority</option>
                        <option value="1" @if($task->priority == 'High') selected @endif>High</option>
                        <option value="2" @if($task->priority == 'Medium') selected @endif>Medium</option>
                        <option value="3" @if($task->priority == 'Low') selected @endif>Low</option>
                    </select>
                </div>
